...
import { normalizeFormData, openWhatsApp } from "@/utils/whatsapp";
...
export default function useWhatsApp(defaultNumber = DEFAULT_WHATSAPP_NUMBER) {
  const sendMessage = useCallback((text, number = defaultNumber) => {
  ...
  const sendForm = useCallback((form, options = {}, number = defaultNumber) => {
    try {
